...
#include <ros.h>
//#include <ros/time.h>
#include <std_msgs/Bool.h> // Used for range messages
...
bool sweep_state = false; // 0 for radar still sweeping. 1 for stop sweeping
//int echo_var = 3;

Servo myServo; // Creates a servo object for controlling the servo motor

void radar_swp_cb( const std_msgs::Bool& state_msg){ //Used to be Float32
...
ros::Subscriber<std_msgs::Bool> sub_state_msg("scan_once_to_whls", radar_swp_cb);
//ros::Publisher servopos("/HerculesUltrasound_Position", &str_msg);
...
void loop() {
  ...
  //}
  //if (sweep_state == true){ // To have servo start sweeping or resume sweeping
